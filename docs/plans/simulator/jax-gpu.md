...
### 追加検証 (pod 9 台目): 障害はデータ転送層に限定される

別データセンター・別 GPU (RTX A5000 / COMMUNITY / $0.16/h) で再検証し、
障害の切り分けを一段進めた。

| 検証 | 結果 |
|---|---|
| SSH 接続 (コマンド実行のみ) | 40 回中 **5 回成功** |
| 接続先 hostname | **常に同一** (`6af9e9668417`) — #5 の host 分散とは別事象 |
| stdin 経由の転送 16KB | **失敗** |
| stdin 経由の転送 128KB / 512KB / 2MB / 8MB | **全て失敗** |

`echo` / `hostname` のようなコマンド実行は通るのに、**16KB のデータすら
stdin で送れない**。つまり障害は「SSH が繋がらない」ではなく
**「セッションで実バイトが流れると切れる」**という転送層の問題。

これにより以下が全て説明できる:

- pip / uv のダウンロードが無言で停止する (受信バイトが流れた時点で切断)
- `setsid nohup` のジョブが死ぬ (親セッションの異常終了に巻き込まれる)
- wheel 直送 (284MB) が 8MB chunk でも通らない

**この層は利用者側から回避できない。** ローカルで wheel を用意する、uv を使う、
image を替える、といった対策は全て「転送さえできれば有効」なものであり、
転送そのものが不可能な状況では効果がない。

累計: pod 9 台、SECURE / COMMUNITY 両方、RTX 4090 / 3090 / A5000、
複数データセンター、pip / uv 両方、複数 image。いずれも GPU 実測に到達せず。

### 追加検証 (pod 12 台目): 障害は時間依存ではない

初回試行から約 75 分後、別データセンター (RTX A5000 / SECURE) で再検証した。
「一時的な障害なら時間を置けば回復する」という仮説の検証。

| 検証 | 結果 |
|---|---|
| SSH 接続 (`echo` のみ) | **0 / 20** |
| stdin 16KB 転送 | 失敗 (25 回試行) |
| pod 自身の `git clone` | 到達できず (コマンド自体が届かない) |

**回復していない。** むしろ 9 台目 (5/40) より悪化している。
時間帯・データセンター・GPU 種別のいずれとも相関せず、アカウント単位か
リージョン全体の問題と考えられる。

### 試したが機能しなかった回避策の一覧

転送層が死んでいるため、以下は全て「転送さえできれば有効」なものであり、
今回の状況では効果がなかった:

1. pod / cloud / GPU / DC / image を替える (12 台)
2. `pip` → `uv` (4.5MB → 4.5GB まで前進したが完了せず)
3. wheel をローカルで取得して直送 (284MB。8MB chunk でも通らず)
4. resumable chunk upload (16KB すら不可)
5. `setsid nohup` によるジョブ detach (親セッション切断で死亡)
6. ジョブ内リトライループ (転送が前提なので無効)
7. pod 自身の `git clone` (コマンドが届かない)
8. `docker_args` による SSH 非依存の self-run
   → **image の entrypoint を上書きしてしまい RunPod runtime が
     未登録 (ports=null) になり、結果を回収できなかった。設計ミス**
