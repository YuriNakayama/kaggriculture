"""ローカルと RunPod pod の間で backend/ ディレクトリを rsync する。
...
DEFAULT_REMOTE_REPO_ROOT = "/workspace/kaggriculture"
DEFAULT_REMOTE_BOT_DIR = f"{DEFAULT_REMOTE_REPO_ROOT}/backend"
...
def build_sync_plan(
    *,
    ...
    Args:
        ...
        local_bot_dir: ローカルの `backend/` 絶対パス。末尾 `/` 付きとして扱う
            (rsync の慣例で、ディレクトリ内容を転送する)。
        direction: "push" (local → pod) または "pull" (pod → local)。
        remote_bot_dir: pod 上の backend ディレクトリ絶対パス。
