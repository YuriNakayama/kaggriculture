...
if [[ "<PREPROCESS_CMD>" == *"parquet_to_npy"* ]]; then
  ...
  # has a 300GB quota that is shared across runs (kaggriculture), so the
  ...
  # iter cb70d34 trap: backend/.venv/bin/python disappeared between
  ...
  if [ ! -x backend/.venv/bin/python ]; then
    VENV_OK=0
    echo "[onstart] WARN: backend/.venv/bin/python missing" >&2
  elif ! backend/.venv/bin/python -c "import pyarrow, torch, numpy" 2>/dev/null; then
    VENV_OK=0
    echo "[onstart] WARN: backend/.venv site-packages incomplete (import pyarrow/torch/numpy failed)" >&2
  fi
  if [ "${VENV_OK}" -eq 0 ]; then
    echo "[onstart] forcing uv sync pre-parquet_to_npy" >&2
    ls -la backend/.venv 2>&1 | head -3 || true
    if [ -L backend/.venv ]; then
      ls -la "$(readlink -f backend/.venv)" 2>&1 | head -5 || true
    fi
    ...
    if ! uv sync --frozen --no-dev --no-default-groups --extra torch-cu124 --directory backend; then
      echo "[onstart] uv sync recovery FAILED" >&2
      mark "55_parquet_to_npy_failed"
      exit 1
    fi
    if [ ! -x backend/.venv/bin/python ] \
       || ! backend/.venv/bin/python -c "import pyarrow, torch, numpy" 2>/dev/null; then
  ...
  if ! ( cd backend && "${PY_BIN}" -m <PREPROCESS_CMD> \
  ...
  if [ "${PARQUET_PRESENT}" -eq 1 ] && [ "${STAGE_UPTODATE}" -eq 1 ]; then
    ...
    # iter11: `uv run` 経由を排除 (race condition 回避)。cwd を backend に揃えて
...
if [ "${PREPROCESS_RAN}" -eq 1 ]; then
  ...
  if [ "${#parquet_files[@]}" -eq 0 ]; then
    ...
    git config user.email "runpod-bot@kaggriculture.local"
