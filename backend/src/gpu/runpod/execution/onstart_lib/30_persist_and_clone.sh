...
mkdir -p /workspace
cd /workspace
if [ ! -d kaggriculture ]; then
  if [ -n "${GIT_PAT:-}" ]; then
    git clone "https://x-access-token:${GIT_PAT}@$(echo "<REPO_URL>" | sed 's|https://||')" kaggriculture
  else
    git clone "<REPO_URL>" kaggriculture
  fi
fi
cd kaggriculture
...
if [ -d /persist ]; then
  ...
  # backend/.venv を /persist/uv-venv-bot に symlink。uv は venv の
  ...
  #   error: failed to remove directory `/workspace/kaggriculture/backend/.venv/lib`:
  ...
  CURRENT_LOCK_HASH=""
  if [ -f backend/uv.lock ] && command -v sha256sum >/dev/null 2>&1; then
    CURRENT_LOCK_HASH=$(sha256sum backend/uv.lock | awk '{print $1}')
  fi
  if [ -n "${CURRENT_LOCK_HASH}" ] \
    ...
    # backend/.venv/bin/python が現存していれば中身は意味がある可能性があるが、
