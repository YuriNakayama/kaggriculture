...
if [ -L backend/.venv ]; then
  echo "[onstart] removing stale /persist symlink backend/.venv -> $(readlink backend/.venv)"
  rm -f backend/.venv
fi
...
if [ "<CASE_FAMILY>" = "reinforce" ]; then
  UV_SYNC_ARGS="--frozen --no-dev --directory backend"
else
  UV_SYNC_ARGS="--frozen --no-dev --no-default-groups --extra torch-cu124 --directory backend"
fi
...
if ! echo "${CUDA_SMOKE_OUT}" | grep -q '"smoke_ok": true'; then
  ...
  # uv 管理 venv には pip バイナリが無い (backend/.venv/bin/pip 不在) ため、
  ...
  uv pip install --python backend/.venv/bin/python --reinstall-package torch \
    --index-url https://download.pytorch.org/whl/cu118 \
    torch || {
      echo "[onstart] FATAL: torch cu118 reinstall failed" >&2
      exit 1
  }
  CUDA_RECHECK=$(backend/.venv/bin/python -c "
import json, torch
...
# iter11 fix: `uv run --project backend` / `uv run --directory backend` を以後の
...
PY_BIN="$(pwd)/backend/.venv/bin/python"
# iter12 fix (case9 連続失敗 2026-05-06 11:28): backend/.venv/bin/dvc shim の
...
# "Project virtual environment directory backend/.venv cannot be used
...
# `uv sync` is declarative — a `--group cuda` sync prunes the
# `dvc[s3]` extra's transitive `s3fs` from the venv, breaking
# subsequent dvc operations. By deferring cuda install to after dvc
# pull, base state stays intact during dvc, and the train process
# gets a venv with the cuda12 plugin (cuda group).
...
case "<CASE>" in
  bench_*_gpu | reinforce_*)
    ...
    if ! ( cd backend && uv sync --no-dev --group cuda --frozen ) >"${UV_CUDA_LOG}" 2>&1; then
      echo "[onstart] install_cuda_jax FAILED — uv sync log tail:" >&2
      tail -30 "${UV_CUDA_LOG}" >&2 || true
      if command -v aws >/dev/null 2>&1; then
        aws s3 cp "${UV_CUDA_LOG}" \
          "s3://${KAGGRICULTURE_DVC_BUCKET}/remote/runpod_artifacts/<RUN_ID>/uv_cuda_sync.log" \
          2>&1 | head -3 || true
      fi
      mark "47_install_cuda_jax_failed"
      exit 1
