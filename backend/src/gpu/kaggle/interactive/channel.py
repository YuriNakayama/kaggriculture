...
コマンド形式 (inbox):
    {
      ...
      "cwd": "/kaggle/working/repo/backend",
      "timeout": 300,
      "env": {"FOO": "bar"}
    }
...
# The bucket is account-specific, so it is never hardcoded here — this
# repository is public. Set KAGGRICULTURE_DVC_BUCKET (or pass an explicit
# bucket) after `terraform apply` reports `dvc_bucket_name`.
DEFAULT_BUCKET = os.environ.get("KAGGRICULTURE_DVC_BUCKET", "")
