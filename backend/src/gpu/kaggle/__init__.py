...
RunPod (``backend/src/gpu/runpod/``) と並ぶもう一つの
GPU 学習基盤。Kaggle Notebooks の無料 GPU 枠 (T4x2 / P100、週 30h) を利用し、
コスト 0 で imitation case を回すサイクルを実現する。

Current layout:
    - cli: Typer command wiring (train / pull / promote / status / ps / logs /
      watch / cost-report / dataset)
    - config: training case registry (runpod_io.config を再利用)
    - dataset: backend/ → Kaggle Dataset への snapshot upload
