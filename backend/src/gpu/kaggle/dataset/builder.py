"""backend/ から Kaggle Dataset 用 snapshot ディレクトリを作る。
...
包含:
    - backend/src/, backend/pipeline/, backend/pyproject.toml,
      backend/uv.lock, params.yaml
...
def build_snapshot(
    ...
    *,
    ...
    Kaggle 側 ``find_repo_root`` が ``(backend/, .git/)`` ペアで repo root を
    認識するため、空の ``<dest_dir>/.git/HEAD`` を必ず作成する。
    ``include_dotenv`` を True にすると ``backend/.env`` を snapshot に同梱する
