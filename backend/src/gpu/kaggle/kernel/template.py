...
学習 run 1 回ごとに生成する notebook。cell 構成:
    ...
    B: pip install /kaggle/input/<dataset>/wheels/*.whl  (pre-built wheels)
    C: pip install -e /kaggle/input/<dataset>/backend/
...
def _cell_env_setup(ctx: RenderContext, run_dir: str, meta_json: str) -> str:
    payload = {
        "Kaggriculture_RUN_ID": ctx.run_id,
        "Kaggriculture_GIT_SHA": ctx.commit_sha,
        "Kaggriculture_GIT_BRANCH": ctx.branch,
        "Kaggriculture_CASE": ctx.case,
        "Kaggriculture_KAGGLE_KERNEL_SLUG": ctx.kaggle_kernel_meta_initial.get(
            "kernel_slug", ""
        ),
        "Kaggriculture_KAGGLE_KERNEL_VERSION": str(
            ctx.kaggle_kernel_meta_initial.get("kernel_version", "")
        ),
        "Kaggriculture_KAGGLE_ACCELERATOR": ctx.accelerator,
        "Kaggriculture_KAGGLE_KERNEL_META": meta_json,
        "Kaggriculture_RUN_DIR": run_dir,
    }
    return (
        "# cell A: env setup\n"
        "import os\n"
...
def _cell_install_wheels(dataset_mount: str) -> str:
    ...
    install any pre-built wheels (if any).
    ...
    contains a normal directory tree (``backend/``, ``data/``,
    ``params.yaml``). We mirror that into ``/tmp/kaggriculture-repo/`` so
    ``find_repo_root()``'s ``(backend/, .git/)`` sentinel pair is satisfied and
    editable installs work.
    """
    return (
        "# cell B: discover dataset mount, mirror to writable REPO_ROOT\n"
        "import os, glob, shutil, subprocess\n"
        "REPO_ROOT = '/tmp/kaggriculture-repo'\n"
        f"EXPECTED = {dataset_mount!r}\n"
        "def _find_mount(target_marker='backend'):\n"
        ...
        "    # the first dir that contains both ``backend`` and ``params.yaml``.\n"
...
def _cell_install_bot(dataset_mount: str) -> str:
    """Make the backend package importable by adding ``backend/src`` and ``backend/`` to
    ...
    """
    return (
        "# cell C: make backend importable + ensure torch covers the assigned GPU\n"
        ...
        "        )\n"
        "\n"
        "# Verify backend package is importable in a subprocess too (matches\n"
...
def _cell_install_jax_cuda() -> str:
    ...
    - `pip install --reinstall jax[cuda12]` は **避ける** (既ロード jax の
      共有ライブラリハンドルが invalid 化して SIGABRT、kaggriculture README
...
def _cell_parquet_to_npy(ctx: RenderContext) -> str:
    ...
    """
    case = ctx.case
    mart_dir = f"/tmp/kaggriculture-repo/data/mart/imitation/{case}"
    return (
        ...
        "env = os.environ.copy()\n"
        "env['PYTHONPATH'] = (\n"
        "    '/tmp/kaggriculture-repo/backend/src:/tmp/kaggriculture-repo/backend:'\n"
        "    '/tmp/kaggriculture-repo'\n"
        ")\n"
        "print('running:', ' '.join(cmd))\n"
        "proc = subprocess.run(\n"
        "    cmd, cwd='/tmp/kaggriculture-repo/backend', env=env, check=False,\n"
        ")\n"
...
def _cell_train(ctx: RenderContext, run_dir: str, dataset_mount: str) -> str:
    ...
    return (
        ...
        "cwd = '/tmp/kaggriculture-repo/backend'\n"
        "env = os.environ.copy()\n"
        "env['PYTHONPATH'] = (\n"
        "    '/tmp/kaggriculture-repo/backend/src:/tmp/kaggriculture-repo/backend:'\n"
        "    '/tmp/kaggriculture-repo'\n"
        ...
        "#  ships sm_75 kernels for T4. Set Kaggriculture_KAGGLE_FORCE_CPU=1 in\n"
        "#  the kernel env to fall back to CPU.)\n"
        "if os.environ.get('Kaggriculture_KAGGLE_FORCE_CPU') == '1':\n"
        "    env['CUDA_VISIBLE_DEVICES'] = ''\n"
        "    print('CUDA disabled via Kaggriculture_KAGGLE_FORCE_CPU')\n"
...
def _cell_collect_artifacts(ctx: RenderContext, run_dir: str) -> str:
    # The train cell sets Kaggriculture_RUN_DIR=run_dir, so train_jax/train write
    ...
    src = f"/tmp/kaggriculture-repo/data/output/models/{family}/{subdir}/runs/{ctx.run_id}/"
    return (
