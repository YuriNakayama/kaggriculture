"""rulebase/case3 -- deterministic scenario-aware economic policy.

Ported verbatim (minus the notebook's ``%%agentfile`` magic) from
``pilkwang__kaggriculture-structured-economic-policy``. The strongest
from-scratch heuristic in the public notebook set, and by a wide margin the
strongest case in this repo: ~149k mean bank versus case2's ~35k.

Unlike the other ports this one needed no correction. Its CROPS / ANIMALS /
MARKET / SHOPS tables and its ``_price_at`` were checked field-by-field against
``kaggriculture.py`` at 1.32.6 and agree exactly, including every price across
the full inventory range.

## Architecture

Not a state machine -- a per-turn six-stage pipeline: observation -> phase and
role assignment -> job generation -> priority/value/travel matching ->
inventory shadow update -> market allocation.

Five phases (``_policy_phase``), checked in order:

* ``LIQUIDATE`` -- final 22 turns
* ``CRISIS``    -- at-risk animals and crops outnumber workers, or shed >= 95
* ``BOOTSTRAP`` -- day <= 4
* ``COMPOUND``  -- day <= 21
* ``REALIZE``   -- otherwise

Jobs are matched to workers by ``S = priority_bonus + value - 8 * manhattan``,
with ``PRIORITY_BONUS`` spanning 120000 down to -100 so that feeding, rescue
watering and terminal harvests are lexicographically protected: growth work only
ever consumes labour those deadlines did not need.

## Notable mechanisms

* **Adaptive melon sizing** (``_melon_target``): 12 tiles when MELON >= $300 and
  the opponent holds <= 5 melon tiles; 8 when price <= 170 or the opponent holds
  >= 12; else 10. This is the only opponent-aware logic in any ported case.
* **Herd ramp**: ``COW, COW, COW, SHEEP`` core, expanding to 11 on day 7 and 15
  on day 11, with no animal purchases after day 18.
* **Reserve fractions** per product (WOOL 0.40, MILK 0.42, MELON 0.58,
  WHEAT 0.68) withhold stock from the market rather than selling everything.
* **Land**: NE and SW on days 5 and 9; SE is never bought.

## The market theory behind it

The notebook derives two results worth keeping in mind for later cases:

1. Ordering ahead of a non-reacting block of ``q`` units moves the *objective*
   by **2x** the price impact, because the margin is a difference of two banks.
2. SELL slots should be ranked by **self-impact** ``I = q * (p(I) - p(I+q))``,
   not gross proceeds -- gross is dominated by base price and so defers cheap
   but steep products until after they have been crushed.

It also notes that withholding inventory lifts the price path for *both* seats,
making it a transfer to the opponent whenever they are the larger supplier in
that window.

## Caveats

* Carries three module-level mutable globals (``_SIGNATURE_ACTIVE``,
  ``_SIGNATURE_LAST_STEP``, ``_SCHEDULE_WHEAT_REQUESTED``) that persist across
  turns. They are re-derived from ``obs`` early each episode rather than
  accumulated, so repeated same-seed episodes in one interpreter were verified
  to reproduce bit-identical results -- but any future edit to how they are
  reset would silently make local benchmarks order-dependent.
* At 2005 lines it is far harder to attribute a score change to a specific
  mechanism than case2 is. Treat it as a strong reference implementation to
  learn from, not as a codebase to tune blind.
"""
...
def _market_actions(obs, config, farm, private, roles, field):
    ...
    for item in PRODUCTS:
        ...
        # Orders are filled unit by unit and interleaved between seats, so on a
        # product both seats sell this turn the earlier slot takes the better
        # prices. Rank by the decay our own order inflicts on its quote -- the
        # slot where arriving late costs most -- rather than by gross proceeds,
        # which is dominated by the base price and carries no information about
        # the local slope.
        impact = quantity * max(
            0.0,
            _price_at(item, inventory, obs)
            - _price_at(item, inventory + quantity, obs),
        )
        sells.append((impact, proceeds, item, quantity))
    sells.sort(reverse=True)
    for _impact, proceeds, item, quantity in sells:
