/**
 * Promise-based wrapper around the game worker. Each Req gets a unique reqId
 * and the matching Res resolves the corresponding pending promise.
 */

import type { Config, GameState } from '../engine/types';
import type { HumanActions, Req, Res, SlotConfig } from './protocol';

type Pending = {
  resolve: (state: GameState) => void;
  reject: (err: Error) => void;
};

export class WorkerClient {
  private worker: Worker;
  private pending = new Map<string, Pending>();
  private nextId = 0;
  /** Per-player error strings from the most recent STEP (e.g. a Python agent raised). */
  lastAgentErrors: Record<number, string> = {};

  constructor(private onProgress?: (message: string) => void) {
    this.worker = new Worker(new URL('./gameWorker.ts', import.meta.url), { type: 'module' });
    this.worker.addEventListener('message', (evt: MessageEvent<Res>) => {
      const res = evt.data;
      if (res.type === 'PROGRESS') {
        this.onProgress?.(res.message);
        return;
      }
      const p = this.pending.get(res.reqId);
      if (!p) return;
      this.pending.delete(res.reqId);
      if (res.type === 'ERROR') p.reject(new Error(res.message));
      else {
        this.lastAgentErrors = res.agentErrors ?? {};
        p.resolve(res.state);
      }
    });
  }

  private nextReqId(): string {
    return String(this.nextId++);
  }

  private dispatch(req: Req): Promise<GameState> {
    return new Promise<GameState>((resolve, reject) => {
      this.pending.set(req.reqId, { resolve, reject });
      this.worker.postMessage(req);
    });
  }

  init(config: Config, numAgents: number, slots: SlotConfig[]): Promise<GameState> {
    return this.dispatch({ type: 'INIT', reqId: this.nextReqId(), config, numAgents, slots });
  }

  step(humanActions: HumanActions): Promise<GameState> {
    return this.dispatch({ type: 'STEP', reqId: this.nextReqId(), humanActions });
  }

  reset(): Promise<GameState> {
    return this.dispatch({ type: 'RESET', reqId: this.nextReqId() });
  }

  getState(): Promise<GameState> {
    return this.dispatch({ type: 'GET_STATE', reqId: this.nextReqId() });
  }

  terminate(): void {
    this.worker.terminate();
  }
}
